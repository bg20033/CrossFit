import { Banknote, CalendarDays, Dumbbell, Target, Users, Wallet } from 'lucide-react'
import { Link } from 'react-router-dom'
import { Button } from '../components/ui/button'

const features = [
  { icon: <Users className="h-5 w-5" />, title: 'Menaxho Anëtarët', text: 'Klientë, trajnerë dhe staf me qasje sipas rolit.' },
  { icon: <CalendarDays className="h-5 w-5" />, title: 'Orare & Grupe', text: 'Organizo grupe CrossFit dhe sesione personale lehtësisht.' },
  { icon: <Dumbbell className="h-5 w-5" />, title: 'Plane & Dieta', text: 'Krijo plane ushtrimesh e dieta, eksporto në PDF.' },
  { icon: <Wallet className="h-5 w-5" />, title: 'Financa', text: 'Ndiq të hyrat, shpenzimet, rrogat dhe faturat.' },
  { icon: <Target className="h-5 w-5" />, title: 'Progres & Qëllime', text: 'Gjurmo peshën, matjet dhe arritjet e klientëve.' },
  { icon: <Banknote className="h-5 w-5" />, title: 'Arka & Recepsion', text: 'Regjistrim i shpejtë, pagesa dhe raporte ditore.' },
]
    ...
    <div className="w-full">
      ...
      <section className="relative overflow-hidden bg-gray-900 text-white">
        <div className="relative mx-auto max-w-7xl px-4 py-24 md:px-6 md:py-32">
          ...
          <h1 className="mt-5 max-w-3xl text-white text-4xl font-extrabold leading-tight md:text-6xl">
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            ...
            <Link to="/about">
              <Button size="lg" variant="outline" className="border-gray-600 bg-transparent text-white hover:bg-gray-800">
                Mëso më shumë
              </Button>
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          ...
          <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {features.map((f) => (
              <div
                key={f.title}
                className="rounded-2xl border border-gray-100 bg-white p-6 transition hover:-translate-y-1"
              >
